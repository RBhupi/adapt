...
def test_setup_output_directories_creates_subdirs(tmp_path):
    """Test that base and logs are created (other dirs created dynamically under RADAR_ID)."""
    dirs = setup_output_directories(tmp_path)

    # Base and logs should exist at root level
    assert tmp_path.is_dir()
    assert (tmp_path / "logs").is_dir()

    # Type-specific dirs are now created dynamically under RADAR_ID/
    # They should NOT exist at root level
...
def test_get_nexrad_path_with_scan_time(tmp_path):
    ...
    # New structure: base/RADAR_ID/nexrad/YYYYMMDD/filename
    # parent = YYYYMMDD, parent.parent = nexrad, parent.parent.parent = RADAR_ID, parent^4 = base
    assert nexrad_path.parent.parent.parent.parent == dirs["base"]
    assert "nexrad" in str(nexrad_path)
...
def test_get_netcdf_path_with_scan_time(tmp_path):
    ...
    # New structure: base/RADAR_ID/gridnc/YYYYMMDD/filename
    # parent = YYYYMMDD, parent.parent = gridnc, parent.parent.parent = RADAR_ID, parent^4 = base
    assert nc_path.parent.parent.parent.parent == dirs["base"]
    assert "gridnc" in str(nc_path)
